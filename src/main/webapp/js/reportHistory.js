...
function showEvents() {
  ...
  var arr=  []; 
  for(var i=1; i<=jsonBom; i++) {
	  var ob = jsonBom[i];
	  var myJSON = JSON.parse(ob);
	  arr.push(myJSON.toString());
	}
  
  if (theDay == -1) return;
  theEvents = data.years[theYear].months[theMonth].days[theDay].events;  
  organizer.list(jsonBom); // what's responsible for listing
}
...
function sendDate(){
	...
	var year= calendar.date.getFullYear().toString(); 
	var sepretor="-";
	var date = year.concat(sepretor, month,sepretor,day );
	$.ajax({
				type : "POST",
				data : date,
				dataType : "json",
				...
				success : function(response) {
					   $("#downfilestatustble").empty();
					   var tt =response.histList;
					   var trHTML = '';
					   trHTML += '<tr style=" background-color: cadetblue;font-family: -webkit-body;font-size: large; color: antiquewhite;"><td>' + 'QID' + '</td><td>' + 'Report File Name' + '</td><td>' + 'Start Time' + '</td><td>' + 'End Time'+ '</td></tr>';
					   $.each(tt, function (i, item) {
				            trHTML += '<tr style=" background-color: #dce5f5d1; color: black; font-size: medium;"><td>' + item.mtime + '</td><td>' + item.text + '</td><td>' + item.startTime + '</td><td>' + item.endTime + '</td></tr>';
				        });
				        $('#downfilestatustble').append(trHTML);
				     
					   $("#fullHeightModalRight").modal("toggle");
					   
					/*  $.each(response.histList, function(i, f) {
                        var myid = 'mytable'+String(idx);
                        idx++;
                        var $table="<table  id='abc' class='mystyles' table border=5><tbody><tr>" + "<td>" + f.mTime + "</td></tr>" + "<tr><td>" + f.text + "</td></tr>" + "<tr><td>" + f.startTime + "</td></tr>" + "<tr><td> " + f.endTime + "</td></tr>" + "</tbody>&nbsp;&nbsp;&nbsp;</table>"
                        $("#downfilestatustble").append($table)
                    });
*/                   
...
organizer.setOnClickListener('days-blocks', function () { sendDate(); console.log("Day block clicked"); }, null);
